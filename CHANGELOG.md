...
- **PRD 75 — Pay Gate UI & Enforcement**: Full-stack pay gate implementation. Paystack provider abstraction layer (`src/lib/payments/`) with HMAC SHA-512 webhook verification and redirect-based checkout. 5 billing UI components: `PaywallBanner`, `UpgradePrompt`, `PricingCard`, `BillingIntervalToggle`, `PricingTierGrid` — all mobile-first, dark+light mode, shadcn/ui. 2 hooks: `usePayGate` (3-layer cascade: feature flag + global setting + per-league override) and `useLeagueSubscription` (subscription state with capacity tracking). 3 API routes: `POST /api/checkout/create-session` (Paystack transaction init), `POST /api/webhooks/paystack` (idempotent webhook processing via `external_payment_id`), `GET /api/leagues/[id]/subscription` (league subscription state). League flows integrated: free tier note on create, paywall banner on overview, invite gating at capacity. 6 analytics events for full conversion funnel (paywall_banner_shown through checkout_completed). 17 files, +1,765 lines.
- **PRD 77 — Alpha→Beta Gate Criteria**: Operational criteria document (`docs/BETA_GATE_CRITERIA.md`) defining 20 measurable pass/fail gates for transitioning from alpha (friends & family) to public beta. 10 HARD gates (non-negotiable) + 10 SOFT gates (informs decision). Sections: quantitative (DAU, D1/D7 retention, submission rate, bugs, errors), qualitative (NPS, unassisted completion, triage backlog, recommendation), infrastructure readiness (payments, monitoring, DB scalability, GDPR/POPIA), feature completeness (core features, mobile performance on 3G, error UX), decision framework (weekly evaluation, aggregation rules, rollback triggers). Progressive beta expansion tiers: 50→200→1,000→GA. All benchmarks cited (indie app/health app sources). PostHog dashboard recommendations included.
- **Paystack Signup Guide**: Step-by-step guide for Paystack account setup with FNB bank account (`docs/artifacts/guide_paystack_signup.md`). Covers pre-flight checklist, signup flow, KYC as Sole Proprietorship, international payments, test API keys, webhook configuration, gotchas (two-step subscription flow, ZAR settlement, name matching), and PRD 76 connection. Designed for both human and browser-agent execution. PRD 76 prerequisite.
...
- **Update Password UX** — Fixed Web Locks deadlock causing "Updating..." to hang forever by switching to stateless Supabase client with 15s timeout. Added success screen after password update instead of immediate redirect. Prevented form from showing on page refresh or direct navigation (sessionStorage recovery flow detection). Added `autocomplete="new-password"` to inputs. Users who navigate to `/update-password` without a recovery flow now see a helpful message instead of the form.
...
- **PRD 48: Universal Health Measurement** ✅ — Architecture doc (`docs/architecture/universal-health-measurement.md`) with conversion table design, DB schema, validation thresholds, AI extraction notes, and implementation phases. Conversion table library (`src/lib/health/conversionTable.ts`) with 6 activity types (steps, calories, kilojoules, swimming, cycling, running), SLP conversion functions, validation, formatting. `ActivityType` union type and `ActivityConfig` interface. 29 unit tests.
- **PRD 47: H2H Leagues** ✅ — Head-to-head league mode schema and algorithm. 3 tables (h2h_seasons, h2h_fixtures, h2h_standings) with RLS policies. League extensions (h2h_enabled, h2h_season_weeks). Season types (fixed/endless) and round types (regular/semifinal/final). Round-robin fixture generator (`src/lib/h2h/fixtureGenerator.ts`) with circle method algorithm, bye week handling, fixture validator. Feature flag `feature_h2h_leagues`. Architecture doc (`docs/architecture/h2h-design.md`). 18 unit tests.
- **PRD 37: Chat DB Foundation** ✅ — Database schema for future chat & social features. 9 tables (chat_conversations, chat_participants, chat_messages, chat_attachments, message_reactions, league_activity, activity_comments, user_blocks, message_reports). RLS policies on all tables. `user_unread_counts` view. `chat-attachments` storage bucket. 3 feature flags (activity_feed, comments, direct_messages) — all disabled by default. `chat_settings` JSONB column on user_preferences. TypeScript types in `src/types/chat.ts` and `src/types/database.ts`.
...
- **PRD 64: Performance Budgets & RUM** ✅ — Web Vitals collection (LCP, INP, CLS) via native PerformanceObserver. Performance budget config with SA mobile allowance (LCP: 3500ms, INP: 250ms, CLS: 0.1). Budget violation detection and separate analytics events. Connection context (effectiveType, RTT, downlink, deviceMemory) for SA user debugging. Supabase preconnect hint in root layout. Initialized via AnalyticsGateProvider when tracking enabled. 19 unit tests.
...
- **Tour Initialization Race Condition Fix** - Fixed critical race condition where tours wouldn't start when navigating with a hash (e.g., `/submit-steps#tour-submit-steps-v1`) because the hash handler ran before TourProvider was fully initialized. Moved hash checking to a separate useEffect with proper dependencies (`i18nReady`, `stateLoaded`) ensuring the tour only starts after all prerequisites are met (i18n loaded, state loaded, startTourRef populated). Added 100ms delay before starting tour to ensure page DOM elements are rendered. Added edge case handling: prevent tour restart if already running, cancel pending tour if user navigates away, comprehensive debug logging for timing diagnostics.
- **Tour Target Elements Missing Fix - Auto Mode Switching** - Fixed critical "Target not mounted" errors where tours would crash on step 2 due to conditional rendering. Comprehensive audit of all 7 tours revealed Submit Steps tour targets single-entry form elements but page defaults to batch mode, and Analytics tour has conditional viewMode rendering. Implemented auto-mode switching solution:
  - **Submit Steps Tour**: Auto-switches to single-entry mode when tour starts, restores original mode (batch/bulk) on completion/skip
  - **Analytics Tour**: Auto-switches to "both" view mode when tour starts, restores original mode (calendar/table) on completion/skip
  - **TourProvider**: Added `originalSubmissionMode` and `originalViewMode` state tracking, modified `startTour()` to detect tours requiring mode switches and dispatch custom events, updated `completeTour()` and `skipTour()` to restore original modes
...
- **Auth**: Implemented robust client-side session recovery (direct cookie parsing) to resolve "Split Authentication State" and prevent Supabase Web Locks deadlocks.
- **Navigation**:
  - Globalized "Submit Steps" (added to Actions menu) for easier access.
  - Robustified SuperAdmin menu visibility by moving permission check to safe AuthContext.
  - Explicitly restricted guest visibility for key actions.
- **Docs**: Added critical warning about Supabase Auth Deadlocks to `AGENTS.md`.
...
- **PRD 18: Documentation** - Created comprehensive system documentation
  - **System Overview** (`docs/FEEDBACK_ROADMAP_SYSTEM.md`) - Architecture, Components, and API reference
