    ...
    if (!isOpen) {
        return (
            <button
                ...
                className="fixed bottom-6 right-6 p-4 bg-[hsl(var(--brand-accent))] hover:bg-[hsl(var(--brand-accent))]/90 text-white rounded-full shadow-lg hover:shadow-[hsl(var(--brand-accent))]/20 transition-all z-50 group"
                title="Open AI Assistant"
            >
                <div className="absolute inset-0 rounded-full animate-ping bg-[hsl(var(--brand-accent))] opacity-20 group-hover:opacity-40" />
        ...
        <div className="fixed bottom-6 right-6 w-96 h-[600px] max-h-[80vh] bg-card border border-border rounded-2xl shadow-2xl flex flex-col z-50 overflow-hidden animate-in slide-in-from-bottom-10 fade-in duration-300">
            ...
            <div className="p-4 border-b border-border bg-black/20 flex items-center justify-between">
                <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-lg bg-[hsl(var(--brand-accent))]/20 flex items-center justify-center text-[hsl(var(--brand-accent))]">
                    ...
                    >
                        <div
                            className={`max-w-[85%] rounded-2xl p-3 text-sm ${msg.role === 'user'
                                ? 'bg-[hsl(var(--brand-accent))] text-white rounded-br-none'
            ...
            <form onSubmit={handleSubmit} className="p-4 border-t border-border bg-black/20">
                <div className="relative">
                    <input
                        ...
                        className="w-full bg-white/5 border border-border rounded-xl pl-4 pr-10 py-3 text-sm text-white focus:outline-none focus:border-[hsl(var(--brand-accent))]/50 transition-colors placeholder:text-white/20"
                    />
                    <button
                        type="submit"
                        disabled={!input.trim() || isLoading}
                        className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 bg-white/10 hover:bg-[hsl(var(--brand-accent))] text-white/60 hover:text-white rounded-lg transition-all disabled:opacity-0"
                        title="Send message"
